...
export let RetrowaveScene = function (scenePath) {
	...
	this.skybox = [
		...
		`./${this.scenePath}skybox/${this.textureResolution}/invisible.png`, // Y-
		`./${this.scenePath}skybox/${this.textureResolution}/invisible.png`, // Z+ 
...
RetrowaveScene.prototype.addPalmtrees = function () {
	...
	// Paltree log
	let logGeometry = new THREE.CylinderBufferGeometry(
		0.25,
		0.125,
		10,
		5,
	...
	// Merge (log + leaves)
	let palmTree = BufferGeometryUtils.mergeBufferGeometries(
		palmTreeConception,
		false
	);
	palmTree.rotateZ(THREE.Math.degToRad(-1.5));

	let palmTreeInstance = new THREE.InstancedBufferGeometry();
	...
	let palmTreePosition = [];

	for (let i = 0; i < 40; i++) {
		let resultLeft = -this.randomize(20, 80, 1); // Left side
		let resultRight = this.randomize(20, 80, 2); // Right side
	...
	let palmTreeMaterial = new THREE.MeshBasicMaterial({
		color: 0x056023,
		side: THREE.DoubleSide,
		wireframe: true,
	});
	...
	let palmTrees = new THREE.Mesh(palmTreeInstance, palmTreeMaterial);
		...
		.then((data) => {
			...
			for (let i = 0; i < this.paths.length; i++) {
				...
				if (this.fillColor !== undefined && this.fillColor !== "none") {
					...
					for (let j = 0; j < this.svgShapes.length; j++) {
